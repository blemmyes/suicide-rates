...
ui <- navbarPage('DASB: Suicide Rates', id='tabs',
                 tabPanel('Overview',
                          sidebarLayout(sidebarPanel(h1('Overview'),
                                                     ...
                                                     p('The hypothesis is, that based on the available information, there should be a correclation between the GDP per capita and the number of suicides in a country. There should also be different suicide rates depending on the generation.'),
                                                     p('The diagram on the right hand side shows which country has data available for which years. Apparantly, some countries have only few years covered. These countries should not be taken into accountwhen it comes to reason about the correalation, as those are not as meaningful as the other countries with more data available.')),
                                        mainPanel(plotOutput(outputId='overview', height=height)))),
                 tabPanel('Histogram',
                          sidebarLayout(sidebarPanel(h1('Histogram'),
                                                     p('To decide which countries should not be taken into consideration, we look at the histogram on the right hand side. It shows the number of years where data is available per country. We decided to concentrate on countries that have at least 10 years of data.')),
                                        mainPanel(plotOutput(outputId='histogram', height=height)))),
                 tabPanel('Country Data',
                          sidebarLayout(sidebarPanel(h1('Country Data'),
                                                    p('In this figure we can see the count of years per country. It shows that there are many countries with over 20 years of data, and only a few that have a small amount of years covered. These are not useful for comparison.')),
                 ...
                 tabPanel('GDP per Capita',
                          sidebarLayout(sidebarPanel(h1('GDP per Capita'),
                                                     p('This plot shows the correlation between the count of suicides per country and the GDP per capita. As the blue line shows, there is no correlation between these two attributes. The R^2 score for this correlation is 0.00166 (p-value of 0.686), that means, only 0.166% of the variance gets explained with the model. This clearly shows no dependency, and the hypotheses of an correlation can be refuted.')),
                 ...
                 tabPanel('Generation',
                          sidebarLayout(sidebarPanel(h1('Generation'),
                                                     p('This plot shows the suicide rates grouped by generation. Each country is represented by a dot. It is interesting to see that the older a generation is, the higher the spead of suicide rates becomes.'),
                                                     h4('Legend:'),
                 ...
                 tabPanel('Generation over Years',
                          sidebarLayout(sidebarPanel(h1('Generation over Years'),
                                                     ...
                                                     p('This shows that the last years (maybe from 2010) should not be taken in consideration as it seems to have missing data. But we can see, for example, that the G.I. generation had the last suicide in the year 2000. This makes sense when considering the G.I.\'s are around 100 years old in 2000! Also the newest generation, the generation Z, does not have so many suicides until now. The first ones happend around the year 2007 where the persons were around maximum 10 years old. It is intersting to see how normaly the suicide rate of each generation is tending to increase over the years.')),
